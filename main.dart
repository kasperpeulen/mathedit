...
main() async {
  ...
  bootstrapStatic(AppComponent, [ROUTER_PROVIDERS, provide(LocationStrategy, useClass: HashLocationStrategy), provide(Firebase, useValue: firebase), provide(Authentication, useValue: await bootstrapAuth(firebase)), provide(GitHub, useFactory: (Authentication auth) => createGitHubClient(auth: auth), deps: [Authentication]), provide(MyGistsService, useClass: MyGistsService), provide(Storage, useValue: window.localStorage), provide(UserService, useClass: UserService), provide(EventBus, useValue: new EventBus()), provide(EditorService, useClass: EditorService), provide(Options, useValue: new Options(texMathDollars: true, texMathSingleBackslash: true)), provide(HtmlWriter, useFactory: (options) => new HtmlWriter(options), deps: [Options]), provide(CommonMarkParser, useFactory: (options) => new CommonMarkParser(options), deps: [Options]), provide(Analytics, useValue: new AnalyticsHtml('UA-40648110-6', 'MathEdit', '0.1.0')..optIn = true)], () { ngStaticInit.initReflector(); });
  bootstrapMathjax();
}
